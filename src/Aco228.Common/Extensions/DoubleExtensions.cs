...
public static class DoubleExtensions
{
    ...
    public static string ToDoubleSinglePrecisionString(this double input, string extension = "")
        => double.IsNaN(input) || double.IsInfinity(input) 
            ? "0" + extension 
            : (int)input == input ? ((int)input).ToString() + extension : $"{input:0.0}" + extension;

    public static string ToDollarString(this double input)
        => input.ToString("$#,##0");
